...
Test.prototype._assertBody = function(body, res) {
  ...
  // parsed
  if ('object' == typeof body && !isregexp && !isbuffer) {
    try {
    ...
    if (body !== res.text) {
      var a = util.inspect(body);
      var b = util.inspect(res.text || res.body);
      ...
      } else if (isbuffer && res.body instanceof Buffer) {
        if (body.compare(res.body) !== 0) {
          return error('expected ' + a + ' response body, got ' + util.inspect(res.body), body, res.body);
        }
      } else {
        return error('expected ' + a + ' response body, got ' + b, body, res.body);
      }
